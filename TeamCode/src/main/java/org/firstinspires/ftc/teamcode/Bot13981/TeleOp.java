...
public class TeleOp extends MainRobot {
    ...
    public void loop() {
        ...
        if (gamepad1.left_bumper && lift_position < 1601) {
            lift_position += 1600; //move lift up
        } else if (gamepad1.left_trigger > 0.5 && lift_position > 1599) {
            lift_position -= 1600; //move lift down
        }

        if (lift_position < get_lift_enc() - 200) {
            drive_lift(-1);
        } else if (lift_position > get_lift_enc() + 50) {
            drive_lift(1);
        } else {
            drive_lift(0);
        }
